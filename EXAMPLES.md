...
- [Examples](#examples)
  ...
  - [6. Protect a route based on specific claims](#6-protect-a-route-based-on-specific-claims)
  - [7. Logout from Identity Provider](#7-logout-from-identity-provider)
  - [8. Validate Claims from an ID token before logging a user in](#8-validate-claims-from-an-id-token-before-logging-a-user-in)
  - [9. Use a custom session store](#9-use-a-custom-session-store)

## 1. Basic setup

The simplest use case for this middleware. By default all routes are protected. The middleware uses the [Implicit Flow with Form Post](https://authok.com/docs/flows/concepts/implicit) to acquire an ID Token from the authorization server and an encrypted cookie session to persist it.
...
app.get('/login', (req, res) => res.oidc.login({ return_to: '/profile' }));
...
Please note that the login and logout routes are not required. Trying to access any protected resource triggers a redirect directly to Authok to login. These are helpful if you need to provide user-facing links to login or logout.
...
If your application needs an [access token](https://authok.com/docs/tokens/access-tokens) for external APIs you can request one by adding `code` to your `response_type`. The Access Token will be available on the request context:
...
[Refresh tokens](https://authok.com/docs/tokens/concepts/refresh-tokens) can be requested along with access tokens using the `offline_access` scope during login. On a route that calls an API, check for an expired token and attempt a refresh:
...
When using an IDP, such as Authok, the default configuration will only log the user out of your application session. When the user logs in again, they will be automatically logged back in to the IDP session. To have the user additionally logged out of the IDP session you will need to add `idpLogout: true` to the middleware configuration.
...
app.use(
  auth({
    idpLogout: true,
    // authokLogout: true // if using custom domain with Authok
  })
);
```
...
In this example, the application is validating the `org_id` to verify that the ID Token was issued to the correct Organization. [Organizations](https://authok.com/docs/organizations) is a set of features of Authok that provide better support for developers who build and maintain SaaS and Business-to-Business (B2B) applications.

If you don't know the Organization upfront, then your application should validate the claim to ensure that the value received is expected or known and that it corresponds to an entity your application trusts, such as a paying customer. If the claim cannot be validated, then the application should deem the token invalid. See https://authok.com/docs/organizations/using-tokens for more info.
