...
interface RequestContext {
   ...
   *
   * See: https://authok.com/docs/protocols/oidc#id-tokens
   */
  idToken?: string;
   ...
   *
   * See: https://authok.com/docs/protocols/oidc#access-tokens
   */
   ...
   *
   * See: https://authok.com/docs/tokens/concepts/refresh-tokens
   */
  refreshToken?: string;
 ...
 *   res.oidc.login({ return_to: '/admin' })
 * })
 * ```
 */
interface ResponseContext {
   ...
   * login routes with custom {@link ConfigParams.authorizationParams authorizationParams} or return_to
   *
   * ```js
   * app.get('/admin-login', (req, res) => {
   *   res.oidc.login({
   ...
   * logout routes with custom return_to
   *
   * ```js
   * app.get('/admin-logout', (req, res) => {
   *   res.oidc.logout({ return_to: '/admin-welcome' })
   * });
   * ```
   */
...
interface LogoutOptions {
  /**
   *  URL to return_to after logout, overrides the Default in {@link ConfigParams.routes.postLogoutRedirect routes.postLogoutRedirect}
   */
  return_to?: string;
}
...
interface ConfigParams {
  ...
  /**
   * Boolean value to enable idpLogout with an Authok custom domain
   */
   ...
   * via the `Authok-Client` header. Default is `true
   */
   ...
   *     return {
   *       return_to: options.return_to || req.originalUrl,
   *       customState: 'foo'
   *     };
   *   }
   * }))
