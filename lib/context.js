...
class ResponseContext {
  ...
  async login(options = {}) {
    ...
    // Set default return_to value, allow passed-in options to override or use originalUrl on GET
    let return_to = config.baseURL;
    if (options.return_to) {
      return_to = options.return_to;
      debug('req.oidc.login() called with return_to: %s', return_to);
    } else if (req.method === 'GET' && req.originalUrl) {
      return_to = req.originalUrl;
      debug('req.oidc.login() without return_to, using: %s', return_to);
    }
  ...
  async logout(params = {}) {
    ...
    let returnURL = params.return_to || config.routes.postLogoutRedirect;
