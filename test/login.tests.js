...
describe('auth', () => {
  ...
  it('should allow custom login route with additional login params', async () => {
    ...
    router.get('/login', (req, res) => {
      res.oidc.login({
        return_to: 'https://example.org/custom-redirect',
        authorizationParams: {
          response_type: 'code',
          response_mode: 'query',
          scope: 'openid email',
    ...
    assert.equal(decodedState.return_to, 'https://example.org/custom-redirect');
  });
  ...
  it('should use a custom state builder', async () => {
    server = await createServer(
      auth({
        ...
        getLoginState: (req, opts) => {
          return {
            return_to: opts.return_to + '/custom-page',
    ...
    assert.equal(decodedState.return_to, 'https://example.org/custom-page');
